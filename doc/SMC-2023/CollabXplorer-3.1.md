...
<!-- 需要论文，课间和课堂录音数据，不具有普适性，需要大量的人工标注数据，不适合大规模的学科领域。 -->
...
在下面的四大分区中，首先是左上方的折线图区域，在当前图中，它指的是每月其他学者进行查询时你被推荐给他们的次数。左下方的柱状图区域指的是根据你的prompt推荐给你的学者，他们按照潜在合作可能性从上至下排列。
...
我们在`Fig.`中展示了两组数据的相似度情况。我们选择以 10 个学者为步长，计算每 10 个数据点的 accuracy 的平均值。可以看到，accuracy 的平均值基本控制在 $(0.75, 0.95)$。通过计算，126 个数据点的 accuracy 的平均值为**90.91%**，方差为<!-- TODO: 方差 -->，这表明我们的学者嵌入模型可以准确地反映学者间的合作关系。
...
<!-- TODO: 双层表头 -->
|           | Recommendation Information |                      |     |     | Satisfaction |     |     |
| --------- | ------------------------- | -------------------- | --- | --- | ------------ | --- | --- |
...
![boxplot](https://web.metattri.com/i/2023/04/16/643b805a5ac0d.png)
<!-- TODO: 详细分析箱线图：均值，上分位点，方差，score1-3的比较，说明哪些问题。。。-->
从箱线图中我们可以看出，以Score1为例，满意度评分处于0.77-0.88之间，中位数是0.85，稳定在一个较高的范围区间内；横向对比数据可以发现，满意度在逐渐降低，这证明我们在前面的预测工作中取得了更好的成绩，即推荐越靠前的学者满意度越高，这个结果表明，本研究中设计的推荐系统在推荐相关契合度高的合作学者方面具有较好的性能和准确性，能够为学者推荐有价值的合作者。
同时，我们也发现了一些推荐系统的不足和改进方向，例如使用效果更好的预训练语义模型作为嵌入，为未来的推荐系统研究提供了一定的参考和借鉴。
...
本文介绍了 CollabXplorer，一个用于跨学科合作发现和推荐的系统。我们的工作和贡献包括一个创新性的学者嵌入模型，用于评估学者的研究方向。通过对数据集进行预处理和分词，我们能够全面地分析学者的出版物，考虑学者的实际研究内容、影响力、合作网络等多个方面，构建了一个更加全面的学者向量表示方法。相比较于目前的系统，我们不需要结构化的数据和大量人工标注，只需要学者的论文即可。这使得我们的系统更具普适性，并且本系统的实现和实际上线运行也具有可行性。在未来，我们将继续改进 CollabXplorer，并将其应用于更广泛的领域中。
