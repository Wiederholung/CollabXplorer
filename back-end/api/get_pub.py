...
import utils.crawler.crawler_utils
...
def paper_manager(paper, result):
    ...
    # 获取摘要
    try:
        result['Article'][article_name]['abstract'] = utils.crawler.crawler_utils.get_abstract_by_crawler(
            result['Article'][article_name]['url'])
    except Exception as e:
        print(e)
...
if __name__ == '__main__':
    # 从 68 继续
    for col in db_utils.get_all_name_en()[68:]:
        # 获取 pid
        dblp_id = get_pid(col)
        ...
        # 将 json 写入数据库
        # db_utils.insert_pub(col, json_dict)
